...
n_normflows_low_rest = [[1, 2, 3, 4]]  # sometimes 20 is not good, then take 8
best_hypers_low_rest = eva.get_best_hyper_autofix(
    datasetname, folders_parameters,
    test_attrs=["best_val_error"],
    n_normflows=n_normflows_low_rest,
    modelnames=("baseline", "baselinedet", "planarflow", "planarflowdet"))

n_normflows_low_radial = [[1, 2, 3]]  # sometimes 20 is not good, then take 8
best_hypers_low_radial = eva.get_best_hyper_autofix(
    datasetname, folders_parameters,
    test_attrs=["best_val_error"],
    n_normflows=n_normflows_low_radial,
    modelnames=("radialflow", "radialflowdet"))

# the following also works for radialflow, as here 20 was almost always not suitable
n_normflows_high_rest = [[8, 20]]
best_hypers_high_rest = eva.get_best_hyper_autofix(
    datasetname, folders_parameters,
    test_attrs=["best_val_error"],
    n_normflows=n_normflows_high_rest,
    modelnames=("baseline", "baselinedet", "planarflow", "planarflowdet"))

n_normflows_high_radial = [[4, 8, 20]]
best_hypers_high_radial = eva.get_best_hyper_autofix(
    datasetname, folders_parameters,
    test_attrs=["best_val_error"],
    n_normflows=n_normflows_high_radial ,
    modelnames=("radialflow", "radialflowdet"))

# TODo radial version
#"radialflow", "radialflowdet"
best_hypers_low = best_hypers_low_rest + best_hypers_low_radial
best_hypers_high = best_hypers_high_rest + best_hypers_high_radial
best_hypers = best_hypers_low + best_hypers_high
...
pprint([(h.modelname, h.best_val_loss, h.best_val_error) for h in best_hypers_high])
print "---------------------------------------------------------"
